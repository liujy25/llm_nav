...
class NavAgent:
    ...
    def __init__(self, cfg=None):
        ...
        self.cfg.setdefault('frontier_visibility_max_depth', self.cfg['bev_max_depth'])
    ...
    def _format_full_messages_as_text(
        ...
        # 3. History rounds info
        history_count = len(self.actionVLM.history) // 2
        lines.append(f"[HISTORY: {history_count} rounds, keeping last {self.cfg['vlm_history']}]")
        lines.append("")
    ...
    def _construct_current_observation(
        ...
        # Build frontier actions text
        if num_actions > 0:
            frontier_actions_text = ", ".join(str(i) for i in range(1, num_actions + 1))
        else:
            frontier_actions_text = "None"
        ...
        prompt = '\n'.join(template_lines).format(
            ...
            frontier_actions=frontier_actions_text,
    ...
    def _construct_fallback_observation(self, iter: int, candidate_traj_ids: list):
        """
        Construct fallback prompt for BEV-only frontier exploration.
        In fallback mode, the model must choose a global frontier ID.
        """
        ...
        if not template_lines:
            template_lines = [
                ...
                "- You have been turning in place without making progress, so you need to choose a frontier from the global BEV map to explore.",
                "- Only one image is provided: BEV map with numbered frontier candidates (green circles with arrows).",
                "- The arrow on each frontier indicates the exploration direction.",
                "- Consider both path length and exploration potential when choosing.",
                "",
                "Available actions:",
                "- MOVE: choose exactly one frontier ID from [{candidate_ids}]",
                "- TURN: None available",
                "",
                "Respond exactly in this format:",
                "",
                "**Action and Reason:** {{Chosen frontier ID and concise reason.}}",
                "**Decision:** {{'action': <frontier_id>}}",
                "",
            ]
    ...
    def _is_frontier_occluded_simple(
        ...
        max_visible_depth = float(self.cfg.get('frontier_visibility_max_depth', self.cfg.get('bev_max_depth', 5.0)))
        min_visible_depth = float(self.cfg.get('bev_min_depth', 0.5))
        if z_pred > max_visible_depth:
            # Frontier itself is beyond the depth observation range, treat as not visible.
            return True
        ...
        valid_mask = (
            ...
            & (depth_patch > min_visible_depth)
            & (depth_patch <= max_visible_depth)
        )
        if not np.any(valid_mask):
            # No valid depth around this projected point in observation range.
            # Keep it conservative: treat as not visible for RGB-guided frontier use.
            return True
    ...
    def _build_fallback_frontier_candidates(self, obs: dict):
        """
        Build MOVE candidates from global BEV frontiers for fallback mode.
        Returns
        -------
        tuple[list, list]
            (candidate_actions, frontier_ids)
            candidate_actions format: [(r, theta, normal_odom), ...]
            frontier_ids: [1, 2, 3, ...] (1-based)
        """
        T_odom_base = obs['base_to_odom_matrix']
        robot_xy = T_odom_base[:2, 3]
        robot_yaw = float(np.arctan2(T_odom_base[1, 0], T_odom_base[0, 0]))

        max_candidates = int(self.cfg.get('fallback_max_frontier_candidates', 8))

        frontiers_odom, normals = self.obstacle_map.get_global_frontiers(robot_xy)

        if len(frontiers_odom) == 0:
            return [], []

        candidates = []
        for i, (frontier_xy, normal) in enumerate(zip(frontiers_odom, normals)):
            dx = float(frontier_xy[0] - robot_xy[0])
            dy = float(frontier_xy[1] - robot_xy[1])
            r = float(np.hypot(dx, dy))
            if r < 0.3:
                continue

            theta_odom = float(np.arctan2(dy, dx))
            theta_base = float(np.arctan2(
                np.sin(theta_odom - robot_yaw),
                np.cos(theta_odom - robot_yaw)
            ))

            # Keep normal conversion consistent with _navigability_from_bev():
            # frontier normals from obstacle_map are in BEV pixel space.
            normal_px = np.array(normal, dtype=np.float64)
            normal_odom = np.array([-normal_px[0], normal_px[1]], dtype=np.float64)
            normal_norm = np.linalg.norm(normal_odom)
            if normal_norm > 1e-6:
                normal_odom = normal_odom / normal_norm
            else:
                # Fallback: point from robot to frontier.
                normal_odom = np.array([dx, dy], dtype=np.float64) / r

            candidates.append((i, r, theta_base, normal_odom))

        if len(candidates) == 0:
            return [], []

        # Sort by distance (closer frontiers first for efficiency)
        candidates.sort(key=lambda x: x[1])
        candidates = candidates[:max_candidates]

        actions = [(r, theta, normal) for _, r, theta, normal in candidates]
        frontier_ids = [idx + 1 for idx, _, _, _ in candidates]  # 1-based
        return actions, frontier_ids
            ...
            if normal_odom is not None:
                ...
                if normal_norm > 1e-6:
                    ...
                    if uv_start is not None and uv_end is not None:
                        # Use projected segment direction directly; no need for both
                        # points to be inside image bounds.
                        du = float(uv_end[0][0] - uv_start[0][0])
                        dv = float(uv_end[0][1] - uv_start[0][1])
                        norm_uv = float(np.hypot(du, dv))

                        # Fallback for degenerate projected segment:
                        # derive a 2D direction from camera-frame normal components.
                        if norm_uv <= 1e-6:
                            normal_base_3d = np.array([normal_base[0], normal_base[1], 0.0], dtype=np.float64)
                            normal_cam = T_cam_base[:3, :3] @ normal_base_3d
                            du = float(intrinsics[0, 0] * normal_cam[0])
                            dv = float(intrinsics[1, 1] * normal_cam[1])
                            norm_uv = float(np.hypot(du, dv))
    ...
    def update_observation_only(self, obs: dict, is_keyframe: bool = True):
        ...
        if self.obstacle_map is not None:
            self.obstacle_map.update_map(
                ...
                min_depth=float(self.cfg.get('bev_min_depth', 0.5)),
                max_depth=float(self.cfg.get('bev_max_depth', 5.0)),
            )
    ...
    def _nav(
        ...
        if fallback_triggered:
            fallback_actions, fallback_frontier_ids = self._build_fallback_frontier_candidates(obs)
            if len(fallback_actions) > 0:
                fallback_mode = True
                self.fallback_active = True
                a_final = fallback_actions
                fallback_traj_ids = fallback_frontier_ids
                print(
                    f"[NavAgent] FALLBACK ACTIVE (frontier mode): consecutive turns={self.consecutive_turn_count}, "
                    f"frontier candidates={len(a_final)}, frontier ids={fallback_frontier_ids}"
                )
            else:
                print(
                    f"[NavAgent] FALLBACK TRIGGERED but no global frontiers "
        ...
        if fallback_mode:
            # In fallback, action number maps to frontier ID, VLM only sees BEV with frontier waypoints.
            a_final_projected = {}
            for waypoint, frontier_id in zip(a_final, fallback_traj_ids):
                if len(waypoint) == 3:
                    r_i, theta_i, normal_odom = waypoint
                ...
                a_final_projected[waypoint_key] = int(frontier_id)

            rgb_vis = obs['rgb'].copy()
            # Show frontier waypoints on BEV map with numbered circles
        ...
        full_text_prompt = current_observation
        ...
        # Call VLM with chat history (images included automatically)
        try:
            ...
            # Simplify the latest user message in history:
            # Replace full prompt text with just "--- Iteration X ---" but keep images
            if len(self.actionVLM.history) >= 2:
                last_user_msg = self.actionVLM.history[-2]  # user message (before assistant)
                new_content = []
                for item in last_user_msg['content']:
                    if item['type'] == 'text':
                        new_content.append({
                            'type': 'text',
                            'text': f'--- Iteration {self.iteration_counter} ---'
                        })
                    else:
                        new_content.append(item)  # keep images
        ...
        try:
            ...
            # Save action to simplified history
            ...
            if fallback_mode:
                ...
                print(f"[NavAgent] FALLBACK selected frontier ID: {action_number}")
            else:
