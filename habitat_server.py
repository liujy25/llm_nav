...
from habitat_pathplanner import ApexNavPyPlanner, PlannerConfig
...
def navigation_reset():
    ...
    # Initialize NavAgent (same decision logic as original server)
...
def navigation_step():
    ...
    # Always update map/trajectory state each frame
                ...
                if is_detection_blocked(
                    ...
                    if verify_result_bool is True:
                        try:
                            ...
                            print(
                                f"[Server] Goal verified by VLM at distance {distance:.2f}m, "
                            ...
                            print(
                                f"[Server] Target in base frame: "
                                f"[{target_pos_base[0]:.3f}, {target_pos_base[1]:.3f}, {target_pos_base[2]:.3f}]"
                            )
                            print(
                                f"[Server] Target in odom frame: "
                                f"[{target_pos_odom[0]:.3f}, {target_pos_odom[1]:.3f}, {target_pos_odom[2]:.3f}]"
                            )
                            print(
                                f"[Server] Robot to target vector (odom): "
                                f"[{target_pos_odom[0]-current_pos_odom[0]:.3f}, "
                            ...
                            print(
                                f"[Server] Final pose: position=({approach_pos[0]:.2f}, {approach_pos[1]:.2f}), "
                            ...
                            cv2.putText(
                                rgb_vis,
                                f"{nav_state['goal']} {confidence:.2f}",
                                (x1, max(0, y1 - 10)),
                                cv2.FONT_HERSHEY_SIMPLEX,
                                0.9,
                                (0, 255, 0),
    ...
    if not is_keyframe:
        ...
        response = (
            f"[Non-keyframe]\n"
            f"Goal: {nav_state['goal']}\n"
            f"Detic verify result: {detic_verify_result}\n"
            f"Action: none\n"
            f"Map updated only."
        )
        with open(os.path.join(nav_state['log_dir'], f'iter_{iteration:04d}_response.txt'), 'w') as f:
    ...
    # Keyframe path: keep original NavAgent decision pipeline
    print("[Server] Running keyframe decision with NavAgent...")
    ...
    planned_path_world: List[List[float]] = []
    ...
    if isinstance(action, tuple) and len(action) == 2 and action[0] == 'turn':
        ...
        # Extra path planning to selected frontier/goal while keeping original action logic.
        # Path planning disabled -- using PointNav only.
        # TODO: re-enable after fixing open-loop action generation from A* path.
        planned_actions = []
        planned_path_world = None
        path_planning_time = 0.0
    ...
    timing_info = {
        ...
        'detic_verify_result': detic_verify_result,
        'blocked_regions_count': int(len(nav_state.get('blocked_detections', []))),
        'path_planning_time': float(path_planning_time),
        'is_keyframe': True
    }
    ...
    if os.path.exists(timing_path):
        ...
        timing_data['path_planning_time'] = float(path_planning_time)
        timing_data['path_waypoints'] = int(len(planned_path_world)) if planned_path_world else 0
        timing_data['planned_actions'] = int(len(planned_actions))
        # Keyframe info already saved above
        with open(timing_path, 'w') as f:
            json.dump(timing_data, f, indent=2)
    ...
    return jsonify({
        ...
        'planned_path': planned_path_world,
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--detic-goals',
        type=str,
        default=['chair', 'plant', 'bed', 'sofa', 'tv_monitor', 'toilet'],
